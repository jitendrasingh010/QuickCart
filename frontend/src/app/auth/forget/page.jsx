...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      try {
        res = await api.post("/userapi/forgot-password", {
          email: trimmedEmail,
        });
      } catch (err) {
      ...
      setStatus({
        type: "success",
        message: res.data?.message || "OTP sent successfully to your email.",
      });
          ...
          <div>
            ...
            <div className="relative group">
              ...
              <input
                ...
                autoFocus
                autoComplete="email"
                className="w-full bg-slate-50/90 dark:bg-slate-800/80 hover:bg-slate-100/80 dark:hover:bg-slate-800 focus:bg-white dark:focus:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-sm font-medium rounded-xl border border-slate-200 dark:border-slate-700 focus:border-blue-600 dark:focus:border-blue-500 focus:ring-4 focus:ring-blue-500/15 transition-all duration-200 outline-none pl-10 pr-4 py-2.5 sm:py-3 shadow-sm"
              />
            </div>
          </div>
